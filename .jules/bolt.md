...
## 2026-06-12 - [Optimizing Creature State Processing with Single-State Fast-Path]
**Learning:** Renting from `ArrayPool<T>` and returning/clearing arrays adds noticeable overhead in high-frequency game logic (such as processing creature states every tick) when the collection contains only one active item. Implementing a fast-path for single-item scenarios bypasses renting/clearing entirely. To do this safely for dictionary-backed states, the struct enumerator must be immediately broken and disposed of *before* the active state is ticked, ensuring that any subsequent modification to the states dictionary (such as adding or removing states inside `state.Tick()`) does not throw `InvalidOperationException`.
**Action:** When collections typically contain zero or one item, provide a dedicated single-item fast-path. Use a non-allocating foreach-break loop to extract the single element and dispose of the enumerator before performing operations that might modify the collection.
