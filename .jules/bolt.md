...
**Learning:** Benchmarks confirm that replacing LINQ `Where().ToList()` with `ArrayPool<T>` and indexed `for` loops for `List<T>` and arrays (hot path) reduces filtering overhead by ~20-75% for small/medium counts (1-10) and eliminates 200-600B of allocations. While `IReadOnlyList` indexing still wins over LINQ, concrete type branches provide the maximum gain by avoiding interface dispatch. A safe `ToList()` fallback for lazy `IEnumerable` is necessary to prevent bugs.
**Action:** Use `ArrayPool<T>` with explicit concrete branches (`List<T>`, `T[]`) for critical hot paths to eliminate filtering allocations and interface dispatch overhead. Retain simple LINQ logic for cold or lazy sequences where safety is prioritized over micro-optimizations.
