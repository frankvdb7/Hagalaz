...
## 2026-06-14 - [Optimizing CSV Parsing with Eager Array APIs]
**Learning:** While optimized `yield return` (lazy) parsing is useful for large streams, many production call sites immediately materialize results via `.ToArray()`. This causes redundant allocations (state machine + collection resizing + array clone). Introducing eager array-returning methods (`DecodeIntValues`, `DecodeDoubleValues`, `DecodeBoolValues`) and using them directly in hot paths (like AutoMapper profiles) eliminates these intermediate allocations. For N=1000, this reduces transient heap pressure by ~50%. Additionally, when returning arrays from CSV parsing, fixed-length consumers (like Skill indexing) must be protected by defensive padding to avoid `IndexOutOfRangeException` when input is null or empty. Consistent whitespace handling (treating whitespace-only input as empty) is more predictable for game configuration data than parsing whitespace as '0'.
**Action:** Expose eager, array-returning versions of parsing utilities (`Decode*Values`) for materialization scenarios while keeping `Select*FromString` lazy for partial enumeration. Always use defensive length-padding (`EnsureLength`) when parsed arrays are indexed by fixed constants. Use `string?` for null-honesty and `string.IsNullOrWhiteSpace` for consistent behavior.
