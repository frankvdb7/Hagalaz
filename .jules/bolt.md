...
**Learning:** While optimized `yield return` (lazy) parsing is useful for large streams, many production call sites immediately materialize the results via `.ToArray()` or `.ToList()`. This causes redundant allocations (iterator state machine + collection resizing + final array clone). Introducing eager array-returning methods (`DecodeIntValues`, `DecodeDoubleValues`) and using them directly in hot paths (like AutoMapper profiles) eliminates these intermediate allocations. For N=1000, this reduces transient heap pressure by ~8.5 KB per call. Additionally, consistent whitespace handling (treating whitespace-only input as empty) provides more predictable behavior than parsing whitespace as a '0' value.
**Action:** Expose eager, array-returning versions of parsing utilities for high-frequency mapping/materialization scenarios. Maintain lazy versions for partial enumeration compatibility. Use `string.IsNullOrWhiteSpace` to consistently return empty collections for empty/whitespace inputs.
