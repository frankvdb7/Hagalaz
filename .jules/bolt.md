...
## 2026-07-17 - [Optimizing Generic Value-Type and Reference-Type Property Setters]
**Learning:** Calling `.Equals()` on a generic value-type constraint `where T : struct` results in boxing (allocating ~48B per check) and reflection-based field comparison via `ValueType.Equals(object?)`. Utilizing `EqualityComparer<T>.Default.Equals(currentValue, newValue)` avoids boxing entirely for equatable structs and primitives, compiling to devirtualized, direct CPU instructions. For reference-type setters, checking `ReferenceEquals` before `.Equals()` bypasses virtual call overhead, yielding a massive ~340x speedup for unchanged state checks.
**Action:** Use `EqualityComparer<T>.Default.Equals` for generic struct equality, and always fast-path reference-equality (`ReferenceEquals`) before invoking virtual `.Equals` on class types.
