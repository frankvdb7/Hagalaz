...
## 2026-05-15 - [Optimizing Base37 and Manual Name Validation]
**Learning:** Correctness is the baseline for performance. Optimization must preserve legacy behavior like Base37 trailing-zero normalization (`while (l % 37L == 0L) l /= 37L`). Using a static 128-byte lookup table for Base37 decoding achieves O(1) character mapping. For encoding, `stackalloc char[12]` combined with `Math.DivRem` and `new string(ReadOnlySpan<char>)` eliminates redundant heap buffers. Manual character loops for validation eliminate `Regex` overhead and achieve zero-allocation performance (~30ns).
**Action:** Always verify round-trip consistency when optimizing codecs. Prefer manual character loops for simple patterns in hot paths to avoid regex engine overhead.
