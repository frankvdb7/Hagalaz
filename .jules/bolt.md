...
## 2026-06-12 - [Optimizing CSV Iterators with Pre-allocated Arrays]
**Learning:** `yield return` iterator blocks for simple CSV parsing create an iterator state machine and multiple heap allocations, which adds overhead compared to returning a pre-allocated array. In `SelectBoolFromString`, delegating to the optimized `DecodeValues` (which uses a single-pass loop) achieved a ~43% speedup. To maintain binary compatibility in core libraries, return the optimized array as `IEnumerable<T>` instead of changing the public signature.
**Action:** Replace `yield return` with single-pass span-based loops and pre-allocated arrays in high-frequency parsing utilities. Always preserve public signatures (e.g., `IEnumerable<T>`) to avoid breaking changes while returning the underlying array.
