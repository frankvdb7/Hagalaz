...
- **[Risk]** Existing clients only encode a world host string. → The status contract and internal world-list model carry the configured TCP port for routing/verification, while the legacy protocol continues to encode the client-compatible host field.
