...
7. **Content-addressed world-list cache.** `WorldInfoService` orders each metadata snapshot deterministically and derives both the HybridCache key and checksum from a stable SHA-256 fingerprint of that snapshot. It does not use process-local counters with shared Redis, so a restarted process cannot reuse a version for different metadata. Character-count-only changes continue to use the separate character-info update path and do not cause full metadata checksum churn.
...
9. **Contacts generation and lease handling.** `WorldSessionStore` owns all observed `(WorldId, InstanceId)` sessions, expires missed leases on a periodic timer, and reports the surviving generation after an offline transition. `IContactSessionService` remains the single owner of contact-session removal and `ContactSignOutMessage` construction. `WorldStatusConsumer` receives that scoped service directly, while the hosted lease-expiry service creates a scope before invoking world cleanup. Bulk cleanup snapshots sessions and uses an atomic key/value compare-and-remove before publishing, so a disconnect/reconnect replacement is never removed by stale cleanup. Each `ContactSessionContext` has an immutable generated identity included in value equality, so a same-world reconnect cannot match the stale snapshot.

10. **Legacy client endpoint compatibility.** The 742 world-list and lobby response wire contracts carry the world host but no per-world port, and the client uses its configured TCP port. The internal status model still validates and records the advertised port, while local Aspire config binds both worlds to the existing client port `443` on distinct loopback hosts (`127.0.0.1` and `127.0.0.2`). Their TCP endpoint annotations explicitly set `isProxied: false`, so DCP does not reserve the same proxied host port for both resources; HTTP and HTTPS endpoints remain proxied and distinct.

11. **Separate game and management bindings.** `World:ListenHost` applies only to the legacy TCP listener. HTTPS and HTTP management listeners always bind `IPAddress.Loopback`, matching Aspire's proxied resource-host target while retaining distinct ports for each world.

12. **Monitor failure isolation.** `WorldStatusService` handles initial status-request failures and each expired-world cleanup operation at their own boundary. Known broker, timeout, invalid-operation, and non-shutdown cancellation failures are logged without exiting the periodic monitor; shutdown cancellation still terminates it normally.
