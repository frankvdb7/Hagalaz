...
- Require positive counts during exact restoration by default. Money pouch explicitly allows its zero-coin item, restores it when persisted pouch state is empty, and rejects non-coin entries. Shop stock has zero-count stock behavior but does not use exact restoration.
