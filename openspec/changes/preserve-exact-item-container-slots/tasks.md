...
- [x] 1.2 Add extra-data, malformed slot/count, zero-coin, empty/non-coin money-pouch, exact restoration, and backing-array ownership tests.
- [x] 1.3 Run the focused tests against the current implementation and record expected failures (11 GameWorld, 2 backing-array, and 2 money-pouch regressions failed before correction).

## 2. Correction

- [x] 2.1 Add the protected restoration boundary and make `SetItems` capacity-safe and array-owning.
- [x] 2.2 Correct all affected dehydrate/hydrate paths; restore the empty money-pouch sentinel and reject non-coin pouch entries.
