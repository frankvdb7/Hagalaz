...
- [x] 1.5 Make prepare invalidation terminate the temporary replacement and carry explicit reconnect success/resynchronization operations.
- [x] 1.6 Discard uncertain pre-loss output, send reconnect output directly through invalidatable replacement-session proxies, and hold normal write admission with the existing write lock.
...
- [x] 3.6 Test deterministic first-packet response ordering, reconnect resynchronization ordering, and invalidated replacement termination.
- [x] 3.7 Test stale-output discard, large direct reconnect output, normal-write admission, keep-alive suppression, and invalidated reconnect senders.
