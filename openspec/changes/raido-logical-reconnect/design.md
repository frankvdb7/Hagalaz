...
12. **Keep the reconnect barrier phase-specific.** `RaidoRebindReservation` exposes only the two operations needed by the handshake boundary: committed response work and post-commit resynchronization work. During either phase, registered work may write through the target while normal logical writes wait. The barrier is released only after the response/resynchronization output and replacement suffix have been ordered; no second transport queue, replay path, or generic transaction coordinator is introduced.
