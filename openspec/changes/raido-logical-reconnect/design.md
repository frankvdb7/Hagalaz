...
5. **Make transfer ownership and ordering explicit.** Each physical pump pair has its own cancellation source, input/output tasks, and stopped completion. A replacement first reserves the reconnecting logical context and its physical session. The session then stops the source generation, waits for the target's previous generation, captures the replacement protocol reader's complete unread suffix, cancels a pending retained-application output flush, and acquires the target's existing write lock. The target remains `Reconnecting` while the replacement session and protocol are attached. Uncertain retained application output is discarded, then a narrow reconnect sender writes the success response with the handshake protocol and authoritative resynchronization with the fresh client protocol directly to the replacement transport. The sender is invalidated after this handoff; it is not ambient state and cannot be reused by a later generation. The suffix is installed, target pumps are started, the target becomes `Connected`, and only then is the write lock released. This establishes `source quiesced -> suffix captured -> target flush quiesced -> logical write lock held -> replacement attached -> uncertain output discarded -> success flushed -> resync flushed -> suffix installed -> target pumps resumed -> Connected -> normal output admitted`; later bytes from the replacement socket cannot overtake the suffix. The reconnect-only flush cancellation is consumed by the write that it unblocks and is not treated as a terminal write failure. Invalidating a prepared reservation aborts its temporary replacement connection, so it cannot fall back to ordinary handshake processing. A failed commit invalidates the reservation and aborts the source or target according to the commit boundary.
...
12. **Keep reconnect output explicit and narrow.** `RaidoRebindReservation` carries the concrete success and resynchronization operations needed by the handshake boundary. Raido supplies each operation an invalidatable `IRaidoClientProxy` tied to the replacement physical session and the appropriate protocol. GameWorld uses the existing `GameSession.SendMessage` seam through a reentrant, synchronous proxy scope for map and appearance updates; other threads wait for that scope and nested sends use the reconnect proxy. Normal Raido writes and pings are blocked by the existing write lock for the whole handoff. No ambient execution context, second output barrier, live application-pipe drain, or generic transaction coordinator is introduced.
