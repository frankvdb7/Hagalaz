...
3. **Policy is state-declared, not container-hard-coded.** `IStateReapplicationPolicy` exposes `KeepExisting`, `Replace`, and `KeepLongestDuration`. The convenience `State` base defaults passive states to `KeepExisting`; `TimedState` defaults timed states to `KeepLongestDuration`. The collection consumes the declared policy and does not compare `TicksLeft` for every state. A generic strategy registry was rejected because there is only one current container and no demonstrated external policy provider.
...
7. **Persistence is opt-in and identity remains metadata-backed.** Only `IPersistentState` instances are dehydrated. The existing `StateMetaDataAttribute` remains the stable ID source for registered persistent states; missing identity causes that state to be omitted rather than crashing generic save. Timed persistent states retain `TicksLeft`; until-removed persistent states use the existing DTO's zero duration. Equipment-derived and activity states are not marked persistent, so equipment remains the authoritative source and transient locks do not become durable character data.
