...
3. **Policy is state-declared, not inherited from lifetime.** `IStateReapplicationPolicy` exposes `KeepExisting`, `Replace`, and `KeepLongestDuration`; `IKeepLongestDurationState` is a separate opt-in capability for the existing timed states that preserve legacy longest-duration behavior. The collection defaults every state without an explicit policy to neutral `KeepExisting`, regardless of lifetime. The collection does not compare `TicksLeft` for passive states. A generic strategy registry was rejected because there is only one current container and no demonstrated external policy provider.
