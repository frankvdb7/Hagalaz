...
7. **Cancel stale skill interactions at the skill boundary.** Mining, Fishing, and Woodcutting register a temporary `CreatureInterruptedEvent` handler while their asynchronous startup data is loading. The handler only records that the interaction was interrupted; the startup checks that state before creating the recurring skill task and unregisters the handler in `finally`. The generic scheduler and `RsAsyncTask` remain creature-agnostic.
