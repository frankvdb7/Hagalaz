...
9. **Keep familiar teardown in the familiar lifecycle.** `NpcService` remains generic and only manages NPC registration, destruction, and storage. `FamiliarScriptBase.OnDestroy` asks its summoner to detach that specific NPC, and `Character.DetachFamiliar(INpc)` clears state only when that NPC is still the active familiar. This prevents a delayed unregister of an older familiar from clearing a newer one.
