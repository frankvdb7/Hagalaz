...
5. **Keep one terminal cleanup implementation.** Successful completion transitions to `completed` and invokes shared interface/input/link reset without refund. Ordinary cancellation transitions only after all escrow is returned successfully; it invokes the same reset method with refund already complete. Destruction first attempts the normal refund and, when capacity prevents it, synchronously stores each offer in the participant's persisted rewards container or bank before terminal cleanup. If compensation is already pending, an exchange does not commit a partial settlement: it removes only exact recipient deltas that are still present, then stores each remaining opposite offer for its intended recipient in that recipient's persisted rewards container or bank. A refund stores remaining value for its original owner. Only after one of those consistent terminal outcomes is secured does the session use the same terminal reset. This prevents a successful exchange from reaching the old refund branch and prevents disconnect cleanup from depending on a future character tick.
