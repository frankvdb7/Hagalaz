...
4. **Apply the exchange as a checked two-recipient operation.** The owner separates coin offers from item offers, validates item capacity and money-pouch overflow capacity for each recipient, and checks every `AddRange`/money-pouch result. The focused trade-only mutation primitive owns the exact applied item/coin delta and validates slot/object identity before rollback; it never reconstructs a trade delta from an aggregate count. This capability is opt-in through the trade participant containers used by inventory, bank, rewards, and the in-memory trade offers; the foundational container and unrelated derived containers keep their existing public removal behavior. Each recipient records only the mutation handles made by the exchange, and a failure compensates those exact deltas without replacing a whole inventory or money pouch. Rollback goes through the container update callback as well, so observers and money-pouch events see the reversal. The money-pouch abstraction owns its pouch/inventory overflow behavior and returns the checked handles needed by trade. The exchange reports compensation-incomplete separately, so completion cannot fall through to refund while a recipient still holds an unremoved trade delta. The offer containers are cleared only after both recipients receive their complete opposite offer, and escrow remains available for one consistent cancellation/refund attempt.
...
5. **Keep one terminal cleanup implementation.** Successful completion transitions to `completed` and invokes shared interface/input/link reset without refund. Ordinary cancellation transitions only after all escrow is returned successfully; it invokes the same reset method with refund already complete. Destruction first attempts the normal refund and, when capacity prevents it, synchronously stores each offer in the participant's persisted rewards container or bank before terminal cleanup. If compensation is already pending, an exchange does not commit a partial settlement: it removes only exact recipient deltas that are still present, then stores each remaining opposite offer for its intended recipient in that recipient's persisted rewards container or bank. A recovery receipt owns both the recovery-destination mutation and the offer removals; if either rollback is incomplete, the receipt remains attached to the pending compensation/session and every retry resolves it before attempting another recovery store. A refund stores remaining value for its original owner. Only after one of those consistent terminal outcomes is secured does the session use the same terminal reset. This prevents a successful exchange from reaching the old refund branch and prevents disconnect cleanup from depending on a future character tick.

## Risks / Trade-offs

- [Risk] A non-standard trade container implementation could report a checked mutation failure after a partial update → the focused mutation records exact applied items and slot identity, compensation-incomplete state blocks refund, the linked session retries compensation, and destruction completes the remaining intended exchange through persisted recovery or preserves the original-owner refund path.
