...
4. **Apply the exchange as a checked two-recipient operation.** The owner separates coin offers from item offers, validates item capacity and money-pouch overflow capacity for each recipient, and checks every `AddRange`/money-pouch result. The focused `ItemContainerMutation` primitive owns the exact applied item/coin delta and validates slot/object identity before rollback; it never reconstructs a trade delta from an aggregate count. Each recipient records only the mutation handles made by the exchange, and a failure compensates those exact deltas without replacing a whole inventory or money pouch. The exchange reports compensation-incomplete separately, so completion cannot fall through to refund while a recipient still holds an unremoved trade delta. The offer containers are cleared only after both recipients receive their complete opposite offer, and escrow remains available for one consistent cancellation/refund attempt.
...
5. **Keep one terminal cleanup implementation.** Successful completion transitions to `completed` and invokes shared interface/input/link reset without refund. Ordinary cancellation transitions only after all escrow is returned successfully; it invokes the same reset method with refund already complete. Destruction first attempts the normal refund and, when capacity prevents it, synchronously stores each offer in the participant's persisted rewards container or bank before terminal cleanup. If compensation is already pending, destruction commits the exact applied mutation delta, stores only the remaining offer value, and then uses the same terminal reset. This prevents a successful exchange from reaching the old refund branch and prevents disconnect cleanup from depending on a future character tick.

## Risks / Trade-offs

- [Risk] A non-standard `IItemContainer` implementation could report a checked mutation failure after a partial update → the focused mutation records exact applied items and slot identity, compensation-incomplete state blocks refund, the linked session retries compensation, and destruction commits the recorded applied delta before recovering the remainder.
