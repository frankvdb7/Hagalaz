...
Protocol instances may contain logical-connection-specific mutable state and must not be shared across unrelated logical connections. Application connection handlers therefore resolve their initial transient protocol inside an async DI scope that remains alive through the logical connection, then pass that runtime instance to the factory. When Hub code selects a different stateful protocol during an invocation, it creates a dedicated async DI scope, resolves the protocol from that scope, and transfers the scope lifetime to the Hub context with the protocol transition. The per-message invocation scope is never the owner of a protocol retained after dispatch. The Hub context disposes the previously transferred lifetime after the write boundary has completed whenever the protocol is replaced, and disposes the current one during logical cleanup; a canceled transition disposes a lifetime it has not transferred. The factory performs no protocol resolution.

The Hub protocol is read-only as a property. A logical protocol transition is an explicit asynchronous operation coordinated by the existing Hub write lock: an in-flight write completes under the old protocol before the transition changes the protocol, later writes use the new protocol, and a read already started with the old protocol is not reinterpreted. Normal Hub dispatch awaits the transition before beginning the next read. `RaidoHubConnectionHandler` uses only the narrow stable-input, terminal, reconnect, and boundary operations delegated by `RaidoHubConnectionContext`; it does not depend directly on `RaidoTcpConnectionContext`.
