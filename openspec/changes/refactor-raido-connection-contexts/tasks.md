...
- [x] 2.5 Update the Hub write/keep-alive paths and connection handler to use the stable transport boundary, drop detached output at the lower boundary, preserve physical failures through reconnect expiry, and ensure terminal cleanup cancels and observes relay tasks
...
- [x] 3.4 Add deterministic coverage for discarding incomplete stable input at a physical replacement boundary while preserving a complete replacement message, and keep boundary acknowledgement separate from the public protocol-reader cursor API
