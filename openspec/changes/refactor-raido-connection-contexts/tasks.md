...
- [x] 2.2 Keep stable heartbeat, infrastructure features, items, lifetime notification, and current inherent-keepalive capability on the TCP context, snapshot initial physical items once, project only the explicit `IConnectionUserFeature` bootstrap feature, do not implicitly promote arbitrary physical/custom or TLS features, preserve features added to the logical Hub context, gate physical heartbeat callbacks by active physical identity and pending input-boundary acknowledgement, observe each physical close token directly, use the handler reader for normal physical input completion, and keep stable close-request ownership with observer-failure containment; verify logical-feature persistence and unknown/TLS non-leakage across replacement
...
Validation note: strict OpenSpec validation, focused Raido tests, the Raido and GameWorld test projects, and the solution build passed. The full solution test command ran with 14 Docker-dependent failures because Docker was unavailable on the host; the non-Docker projects passed.
