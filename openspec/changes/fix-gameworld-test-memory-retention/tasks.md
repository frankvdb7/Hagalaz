...
- [ ] 3.1 Run the focused GameWorld worker and pathfinder tests, then the complete `Hagalaz.Services.GameWorld.Tests` project with serialized, non-overlapping execution; verify the intended test counts pass and no testhost remains running after completion. The current source project run remains pending because the project driver exceeded safe memory limits; the existing compiled assembly passed 811/811.
- [ ] 3.2 Run the broader relevant test projects, validate the OpenSpec change strictly, and review the cumulative diff for public API changes, global parallelism changes, GC workarounds, disabled tests, or stranded background work. OpenSpec and focused diff review pass; broader project execution remains pending with the same project-driver memory limitation.
