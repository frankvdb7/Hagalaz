...
namespace Hagalaz.Game.Scripts.Skills.Summoning
{
    ...
    {
        public PouchScreen(ICharacterContextAccessor characterContextAccessor, IItemService itemService, ISummoningService summoningService, ISummoningSkillService summoningSkillService) : base(characterContextAccessor)
        {
        ...
        {
            InterfaceInstance.AttachClickHandler(19, (componentID, type, itemID, slot) =>
            {
                ...
                else if (type == ComponentClickType.Option5Click)
                {
                    _pouchXHandler = Owner.Widgets.IntInputHandler = value =>
                    {
                        ...
                        if (_summoningSkillService.CanCreatePouch(Owner, definition, itemID, value))
                        {
                            ...
                            {
                                Owner.Inventory.Remove(new Item(key, iAmount * value));
                            }

                            Owner.Inventory.Add(new Item(itemID, value));
                ...
                if (amount > 0)
                {
                    if (_summoningSkillService.CanCreatePouch(Owner, definition, itemID, amount))
                    {
                        ...
                        if (requirements != null)
                        {
                            ...
                            {
                                var iAmount = pair.Value;

                                Owner.Inventory.Remove(new Item(pair.Key, iAmount * amount));
                            }
                        }

                        Owner.Inventory.Add(new Item(itemID, amount));
            ...
            Owner.Configurations.SendCs2Script(757, [(672 << 16) | 16, 8, 10, "Infuse<col=FF9040>", "Infuse-5<col=FF9040>", "Infuse-10<col=FF9040>", "Infuse-All<col=FF9040>", "Infuse-X<col=FF9040>", "List<col=FF9040>", 1, 78
            ]);
