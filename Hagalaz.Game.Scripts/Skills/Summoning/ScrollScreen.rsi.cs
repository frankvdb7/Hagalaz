...
namespace Hagalaz.Game.Scripts.Skills.Summoning
{
    ...
    public class ScrollScreen : WidgetScript
    {
        ...
        public ScrollScreen(ICharacterContextAccessor characterContextAccessor, IItemService itemService, ISummoningService summoningService, ISummoningSkillService summoningSkillService) : base(characterContextAccessor)
        {
        ...
        public override void OnOpen()
        {
            ...
            InterfaceInstance.AttachClickHandler(18, (componentID, type, itemID, slot) =>
            {
            ...
            InterfaceInstance.AttachClickHandler(16, (componentID, type, itemID, itemSlot) =>
            {
                if (itemSlot < 0)
                {
                    return false;
                ...
                else if (type == ComponentClickType.Option5Click)
                {
                    _scrollXHandler = Owner.Widgets.IntInputHandler = value =>
                    {
                        ...
                        {
                            var removed = Owner.Inventory.Remove(new Item(definition.PouchId, value));
                            if (removed > 0)
                            {
                                Owner.Inventory.Add(new Item(definition.ScrollId, removed * _scrollsPerPouch));
                ...
                if (amount > 0)
                {
                    if (_summoningSkillService.CanCreateScroll(Owner, definition))
                    {
                        var removed = Owner.Inventory.Remove(new Item(definition.PouchId, amount));
                        if (removed > 0)
                        {
                            Owner.Inventory.Add(new Item(definition.ScrollId, removed * _scrollsPerPouch));
                            Owner.Statistics.AddExperience(StatisticsConstants.Summoning, definition.ScrollExperience * removed);
            ...
            Owner.Configurations.SendCs2Script(763, [(666 << 16) | 16, 8, 10, "Transform<col=FF9040>", "Transform-5<col=FF9040>", "Transform-10<col=FF9040>", "Transform-All<col=FF9040>", "Transform-X<col=FF9040>", 1, 78
            ]);
