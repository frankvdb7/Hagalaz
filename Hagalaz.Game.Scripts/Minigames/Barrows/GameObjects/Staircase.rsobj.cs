using Hagalaz.Game.Abstractions.Model;
using Hagalaz.Game.Abstractions.Model.Creatures.Characters;
using Hagalaz.Game.Abstractions.Model.GameObjects;
using Hagalaz.Game.Abstractions.Tasks;
using Hagalaz.Game.Scripts.Model.GameObjects;

namespace Hagalaz.Game.Scripts.Minigames.Barrows.GameObjects
{
    /// <summary>
    /// </summary>
    public class Staircase : GameObjectScript
    {
        /// <summary>
        ///     Happens when character click's this object and then walks to it
        ///     and reaches it.
        ///     This method is called by OnCharacterClick by default, if OnCharacterClick is overrided
        ///     than this method is not guaranteed to be called.
        /// </summary>
        /// <param name="clicker">Character which clicked on the object.</param>
        /// <param name="clickType">Type of the click that was performed.</param>
        public override void OnCharacterClickPerform(ICharacter clicker, GameObjectClickType clickType)
        {
            if (clickType == GameObjectClickType.Option1Click)
            {
                clicker.Interrupt(true);
                clicker.Movement.Lock(true);
                clicker.QueueAnimation(Animation.Create(828));
                clicker.QueueTask(new RsTask(() =>
                    {
                        ILocation teleport = null;
                        var id = Owner.Id;
                        switch (id)
                        {
                            // ahrim
                            case 6702:
                                teleport = BarrowsConstants.AhrimSpades;
                                break;
                            // dharok
                            case 6703:
                                teleport = BarrowsConstants.DharokSpades;
                                break;
                            // guthan
                            case 6704:
                                teleport = BarrowsConstants.GuthanSpades;
                                break;
                            // karil
                            case 6705:
                                teleport = BarrowsConstants.KarilSpades;
                                break;
                            // torag
                            case 6706:
                                teleport = BarrowsConstants.ToragSpades;
                                break;
                            // verac / akrisae
                            case 6707:
                                teleport = BarrowsConstants.VeracSpades;
                                break;
                        }

                        clicker.Movement.Teleport(Teleport.Create(teleport));
                        clicker.Movement.Unlock(false);
                    }, 2));
                return;
            }

            base.OnCharacterClickPerform(clicker, clickType);
        }

        /// <summary>
        ///     Get's objectIDS which are suitable for this script.
        /// </summary>
        /// <returns></returns>
        public override int[] GetSuitableObjects() => [6702, 6703, 6704, 6705, 6706, 6707];

        /// <summary>
        ///     Get's called when owner is found.
        /// </summary>
        protected override void Initialize()
        {
        }
    }
}