        ...
        public override void OnCharacterClickPerform(ICharacter clicker, GameObjectClickType clickType)
        {
            if (clickType == GameObjectClickType.Option1Click)
            {
                if (clicker.Location.Y == 5332)
                {
                    ...
                    if (script.ZamorakKills < 20)
                    {
                        clicker.SendChatMessage("You need 20 Zamorak kills to pass through this door.");
                        return;
                    }

                    script.ResetZamorakKills();
                }

                clicker.Movement.Lock(true);
                clicker.QueueTask(new RsTask(() =>
                    {
                        clicker.Movement.Teleport(Teleport.Create(Location.Create(clicker.Location.X, (short)(clicker.Location.Y == 5332 ? 5331 : 5332), clicker.Location.Z, 0)));
