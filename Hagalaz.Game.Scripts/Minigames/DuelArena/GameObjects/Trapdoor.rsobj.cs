...
namespace Hagalaz.Game.Scripts.Minigames.DuelArena.GameObjects
{
    ...
    public class Trapdoor : GameObjectScript
    {
        ...
        public override void OnCharacterClickPerform(ICharacter clicker, GameObjectClickType clickType)
        {
            ...
            {
                var script = clicker.GetScript<DuelArenaCombatScript>();
                if (script != null)
                {
                    if (script.Rules[DuelRules.Rule.NoForfeit])
                    {
                        clicker.SendChatMessage("Forfeit has been disabled.");
                        return;
                    }
                }
                ...
                clicker.QueueTask(new RsTask(() =>
                    {
                        script?.StartVictoryStage(script.Target, script.Character);

                        clicker.Movement.Teleport(Teleport.Create(DuelData.GetDuelLobbyLocation(regionService)));
