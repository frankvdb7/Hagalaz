...
public sealed class FamiliarFactoryTests
{
    ...
    [TestMethod]
    public void Spawn_WhenRegistrationFails_DetachesFamiliarAndDoesNotReturnAHandle()
    {
        var builder = Substitute.For<INpcBuilder>();
        var npcId = Substitute.For<INpcId>();
        var npcLocation = Substitute.For<INpcLocation>();
        ...
        Assert.ThrowsExactly<InvalidOperationException>(() => factory.Spawn(summoner, definition));
    ...
    [TestMethod]
    public void TryRestore_WhenRegistrationFails_DetachesFamiliarAndClearsPendingState()
    {
        var builder = Substitute.For<INpcBuilder>();
        var npcId = Substitute.For<INpcId>();
        var npcLocation = Substitute.For<INpcLocation>();
        ...
        var definitions = Substitute.For<ISummoningDefinitionStore>();
        var restorationState = new FamiliarRestorationState();
        var restoredState = new HydratedFamiliarDto { FamiliarId = definition.NpcId, TicksRemaining = 42 };
        Func<INpcScriptActivator, INpc, INpcScript>? scriptFactory = null;

        restorationState.SetFamiliar(restoredState);
        summoner.Location.Returns(location);
        script.Familiar.Returns(familiar);
        builder.Create().Returns(npcId);
        npcId.WithId(definition.NpcId).Returns(npcLocation);
        ...
        Assert.ThrowsExactly<InvalidOperationException>(() => factory.TryRestore(summoner));
