...
namespace Hagalaz.Cache.Logic.Codecs
{
    public class ClientMapDefinitionCodec : ITypeCodec<IClientMapDefinition>
    {
        ...
        {
            if (opcode == 1)
                definition.KeyType = Encoding.ASCII.GetChars([(byte)(stream.ReadSignedByte() & 0xFF)])[0];
            else if (opcode == 2)
                definition.ValType = Encoding.ASCII.GetChars([(byte)(stream.ReadSignedByte() & 0xFF)])[0];
            else if (opcode == 3)
