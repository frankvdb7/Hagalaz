        ...
        public override void OnKilledBy(ICreature killer)
        {
                     ...
                     itemsOnDeath.droppedItems.Where(item => item.ItemScript.CanTradeItem(item, _character))) // check the current item owner if it can trade
            {
                var groundItem = _groundItemBuilder.Create()
                    ...
                    .Build();
                _mapRegionService.GetOrCreateMapRegion(Owner.Location.RegionId, Owner.Location.Dimension, true)
            ...
            var bones = _groundItemBuilder.Create()
                ...
                .WithOwner(groundItemOwner)
                .Build();
            _mapRegionService.GetOrCreateMapRegion(Owner.Location.RegionId, Owner.Location.Dimension, true).Add(bones);
