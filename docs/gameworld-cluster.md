...
The checked-in Aspire configuration runs both GameWorld resources on the existing client TCP port `443`, with world 1 bound and advertised as `127.0.0.1` and world 2 bound and advertised as `127.0.0.2`. Their TCP Aspire endpoints are explicitly proxyless target-port endpoints, so DCP does not try to allocate the same proxied host port for both resources; Kestrel owns the distinct loopback bindings. HTTPS/HTTP management listeners remain on `127.0.0.1` for Aspire's proxied resource-host target, with ports `7010/5010` and `7011/5011`, and world IDs `1` and `2`. The two resources share the existing database, RabbitMQ, and Redis dependencies but advertise distinct client hosts.
