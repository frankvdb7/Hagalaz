...
namespace Raido.Server
{
    public class RaidoConnectionHandler
    {
        ...
        public RaidoConnectionHandler(
            ...
            IRaidoLifetimeManager lifetimeManager, IRaidoDispatcher dispatcher)
        {
        ...
        public async Task ConnectAsync(RaidoConnectionContext connection)
        {
            ...
            finally
            {
                ...
                Log.ConnectedEnding(_logger, connection);
        ...
        private static class Log
        {
            private static readonly Action<ILogger, string, Exception> _errorDispatchingEvent = LoggerMessage.Define<string>(LogLevel.Error,
                new EventId(1, "ErrorDispatchingEvent"), "Error when dispatching '{Method}' on hub.");

            private static readonly Action<ILogger, Exception> _errorProcessingRequest =
                LoggerMessage.Define(LogLevel.Debug, new EventId(2, "ErrorProcessingRequest"),
            ...
            private static readonly Action<ILogger, string, Exception?> _connectedEnding =
                LoggerMessage.Define<string>(LogLevel.Debug, new EventId(4, "ConnectedEnding"), "OnConnectedAsync ending for connection '{ConnectionId}'.");

            private static readonly Action<ILogger, string, Exception?> _receivedMessage =
                LoggerMessage.Define<string>(LogLevel.Trace, new EventId(5, "ReceivedMessage"), "Received a {0} from the client.");
            ...
            public static void ConnectedEnding(ILogger logger, RaidoConnectionContext connectionContext) =>
                _connectedEnding(logger, connectionContext.ConnectionId, null);

            public static void ReceivedMessage(ILogger logger, RaidoMessage message) => 
                _receivedMessage(logger, message.GetType().Name , null);
        }
    }
}